...
function Cards({ className, ...props }: React.ComponentProps<"div">) {
    return (
        <div
            data-slot="card"
            className={cn(
                "flex flex-col rounded-2xl card-border-linear-gradient sm:sticky sm:top-25 h-full max-w-5xl relative",
                className,
            )}
            {...props}
        />
...
function CardTitle({ className, ...props }: React.ComponentProps<"h3">) {
    return (
        <h3
            ...
            className={cn(
                "lg:text-[1.625rem] text-xl font-extrabold font-brico-gro text-brand-fine-blue border-b-brand-lavender border-b-2 pb-2 tracking-[0.03em] md:text-center lg:text-left",
                className,
            )}
            {...props}
        />
